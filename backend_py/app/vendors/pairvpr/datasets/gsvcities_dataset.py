# This file has been modified from GSVCitiesDataloader.py in https://github.com/amaralibey/gsv-cities
#
#
...
def load_train_dataset(cfg, dsetroot: str):
    ...
    dataset_location = os.path.join(dsetroot, cfg.dataset_locations.gsv)
    ...
    dataset = GSVCitiesDataset(
        ...
        base_path=dataset_location
    )
    return dataset


def load_val_dataset(cfg, dsetroot: str, src_root_dir: Path, val_set_names) -> List:
    mean_dataset = IMAGENET_MEAN_STD['mean']
    std_dataset = IMAGENET_MEAN_STD['std']
    ...
    for valid_set_name in val_set_names:
        if valid_set_name.lower() == 'msls_val':
            ...
            # pair-vpr only uses MSLS-val as a val set. If you want a different val set please add a feature request.
